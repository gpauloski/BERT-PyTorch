...
def write_instance_to_example_file(instances, tokenizer, max_seq_length,
    ...
    f.create_dataset("input_ids", data=features["input_ids"], 
            dtype='i4', compression='gzip')
    f.create_dataset("input_mask", data=features["input_mask"], 
            dtype='i1', compression='gzip')
    f.create_dataset("segment_ids", data=features["segment_ids"], 
            dtype='i1', compression='gzip')
    f.create_dataset("masked_lm_positions", 
            data=features["masked_lm_positions"], dtype='i4', compression='gzip')
    f.create_dataset("masked_lm_ids", data=features["masked_lm_ids"], 
            dtype='i4', compression='gzip')
...
def create_training_instances(input_files, tokenizer, max_seq_length,
    ...
    print("[encoder] Creating instances from {}".format(input_files))
        ...
        if i == len(document) - 1 or current_length >= target_seq_length:
            if current_chunk:
                ...
                if len(current_chunk) == 1 or rng.random() < 0.5:
                    ...
                    # make sure that the random document is not the same as the
                ...
                tmp = create_masked_lm_predictions(tokens, masked_lm_prob, 
                        max_predictions_per_seq, vocab_words, rng)
                (tokens, masked_lm_positions, masked_lm_labels) = tmp 
...
def process(input_files,
    ...
    tokenizer = tokenization.BertTokenizer(vocab_file, 
            do_lower_case=do_lower_case, max_len=512)
    ...
    write_instance_to_example_file(instances, tokenizer,
            max_seq_length, max_predictions_per_seq, output_file)
    print('[encoder] Processed {} (time={:.0f}s)'.format(
...
def main():
    ...
    parser.add_argument("--dupe_factor", default=10, type=int,
                        help='Number of times to duplicate the input data '
    ...
    output_prefix += '_max_pred_' + str(args.max_predictions_per_seq)
    output_prefix += '_masked_lm_prob_' + str(args.masked_lm_prob)
    output_prefix += '_dupe_factor_' + str(args.dupe_factor)
    output_prefix += '_bert_tokenizer'
