...
import src.modeling_cuda_graph as modeling
...
def parse_arguments():
    ...
    parser.add_argument('--cuda_graphs', default=False, action='store_true',
                        help="Use CUDA Graphs")
...
def main(args):
    ...
    if args.cuda_graphs:
        ...
        static_input = (
            torch.randint_like(example_batch[0], low=0, 
                               high=model.config.vocab_size, device=args.device),
            torch.zeros_like(example_batch[1], device=args.device),
            torch.randint_like(example_batch[2], low=0, high=2, 
                               device=args.device)
        )
        if scaler is not None:
            with autocast():
