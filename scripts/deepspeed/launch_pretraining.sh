...
conda activate /lus/theta-fs0/projects/SuperBERT/jgpaul/envs/deepspeed

export NCCL_DEBUG="INFO"
TORCH_EXTENSIONS_DIR=/lus/theta-fs0/projects/SuperBERT/jgpaul/DeepSpeed/torch-extensions
...
if [[ "$NNODES" -eq 1 ]]; then
    python -m torch.distributed.launch --nproc_per_node=$NGPUS \
        run_pretraining_deepspeed.py $KWARGS
else
    python -m torch.distributed.launch \
        --nproc_per_node=$NGPUS --nnodes=$NNODES \
        --node_rank=$LOCAL_RANK --master_addr=$MASTER \
    run_pretraining_deepspeed.py \
        --config results/deepspeed_test/bert_phase_1_config.json \
        --model_config results/deepspeed_test/bert_large_uncased_config.json
fi
